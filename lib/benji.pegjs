  ...
  function mkTree(elements) {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      if (element.indent <= 0) {
        ...
        if (prev && prev.type === 'Cmd' && /^repeat|^maybe$/.test(prev.cmd)) {
          prev.maybe = seq;
        }
...
// Simple elements
Simple = Dialog / Play / Repeat / RepeatTime / RepeatUntil / Set / Maybe / Else
...
// Maybe/else command
Maybe = ":maybe" __ chance:[0-9]+ "%" { return mkCmd('maybe', flatten(chance)); }
